...
static void pusb_conf_options_get_from(t_pusb_options *opts, const char *from,
                                       xmlDoc *doc) {
...
static int pusb_conf_parse_options(t_pusb_options *opts, xmlDoc *doc,
  ...
  struct s_opt_list opt_list[] = {{CONF_DEVICE_XPATH, opts->device.name},
...
static int pusb_conf_device_get_property(t_pusb_options *opts, xmlDoc *doc,
                                         const char *property, char *store,
                                         size_t size, char *deviceId) {
  char *xpath = NULL;
  size_t xpath_len;
  int retval;

  xpath_len =
      strlen(CONF_DEVICE_XPATH) + strlen(deviceId) + strlen(property) + 1;
...
static int pusb_conf_parse_device(t_pusb_options *opts, xmlDoc *doc,
                                  int deviceIndex, char *deviceId) {
  pusb_conf_device_get_property(
      opts, doc, "vendor", opts->device_list[deviceIndex].vendor,
      sizeof(opts->device_list[deviceIndex].vendor), deviceId);
  pusb_conf_device_get_property(
      opts, doc, "model", opts->device_list[deviceIndex].model,
      sizeof(opts->device_list[deviceIndex].model), deviceId);

  if (!pusb_conf_device_get_property(
          opts, doc, "serial", opts->device_list[deviceIndex].serial,
          sizeof(opts->device_list[deviceIndex].serial), deviceId)) {
    return (0);
  }

  pusb_conf_device_get_property(
      opts, doc, "volume_uuid", opts->device_list[deviceIndex].volume_uuid,
      sizeof(opts->device_list[deviceIndex].volume_uuid), deviceId);
  return (1);
}
...
int pusb_conf_init(t_pusb_options *opts) {
  ...
  snprintf(opts->system_pad_directory, sizeof(opts->system_pad_directory), "%s",
           ".pamusb");
  snprintf(opts->device_pad_directory, sizeof(opts->device_pad_directory), "%s",
           ".pamusb");
...
int pusb_conf_parse(const char *file, t_pusb_options *opts, const char *user,
                    const char *service) {
  xmlDoc *doc = NULL;
  int retval;
  ...
  retval = pusb_xpath_get_string_list(doc, device_xpath, device_list,
  ...
  for (int currentDevice = 0; currentDevice < 10; currentDevice++) {
    ...
    strncpy(opts->device_list[currentDevice].name, device_list[currentDevice],
            strnlen(device_list[currentDevice], 128));
