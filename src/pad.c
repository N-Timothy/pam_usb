...
static FILE *pusb_pad_open_device(t_pusb_options *opts, const char *mnt_point,
                                  const char *user, const char *mode) {
  FILE *f;
  char
      path_devpad[(sizeof(mnt_point) + sizeof(opts->device_pad_directory) + 2)];
  char path_userpad[(sizeof(mnt_point) + sizeof(opts->device_pad_directory) +
                     sizeof(opts->hostname) + sizeof(user) + 7)];
  struct stat sb;

  memset(path_devpad, 0x00, sizeof(path_devpad));
  memset(path_userpad, 0x00, sizeof(path_userpad));

  snprintf(path_devpad, sizeof(path_devpad), "%s/%s", mnt_point,
...
static void pusb_pad_update(t_pusb_options *opts, const char *volume,
                            const char *user) {
  FILE *f_device = NULL;
  FILE *f_system = NULL;
  char magic[1024];
  ...
  /**
   * In case you wonder, how I did, if this should use /dev/urandom instead: no,
   * /dev/random is correct in this case See
   * https://crypto.stackexchange.com/a/35032
   */
...
int pusb_pad_check(t_pusb_options *opts, UDisksClient *udisks,
                   const char *user) {
