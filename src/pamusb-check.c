...
static void pusb_check_usage(const char *name) {
  fprintf(stderr,
          "Usage: %s [--help] [--debug] [--config=path] [--service=name] "
          "[--dump] [--quiet] [--debug] [--version] <username>\n",
          name);
}
...
int main(int argc, char **argv) {
  ...
  struct option long_options[] = {{"help", 0, 0, 0},    {"config", 1, 0, 0},
                                  {"service", 1, 0, 0}, {"dump", 0, 0, 0},
                                  {"quiet", 0, 0, 0},   {"debug", 0, 0, 0},
  ...
  while ((opt = getopt_long(argc, argv, short_options, long_options,
    ...
    if (opt == 'h' || (!opt && !strcmp(long_options[opt_index].name, "help"))) {
      ...
      return (1);
               ...
               (!opt && !strcmp(long_options[opt_index].name, "config"))) {
      conf_file = optarg;
    } else if (opt == 's' ||
               (!opt && !strcmp(long_options[opt_index].name, "service"))) {
      service = optarg;
    } else if (opt == 'd' ||
               (!opt && !strcmp(long_options[opt_index].name, "dump"))) {
      dump = 1;
    } else if (opt == 'q' ||
               (!opt && !strcmp(long_options[opt_index].name, "quiet"))) {
      quiet = 1;
    } else if (opt == 'D' ||
               (!opt && !strcmp(long_options[opt_index].name, "debug"))) {
      debug = 1;
    } else if (opt == 'v' ||
               (!opt && !strcmp(long_options[opt_index].name, "version"))) {
