...
int pusb_xpath_get_string_list(xmlDocPtr doc, const char *path, char *values[],
                               size_t size) {
  xmlXPathObject *result = NULL;
  xmlNode *node = NULL;
  xmlChar *result_string = NULL;
  ...
  for (int currentResult = 0; currentResult < result->nodesetval->nodeNr;
       currentResult++) {
    ...
    if (!pusb_xpath_strip_string(values[currentResult], (char *)result_string,
                                 size)) {
      //	log_debug("Result for %s (%s) is too long (max: %d)\n", path,
      //(const char *)result_string, size);
      continue;
...
int pusb_xpath_get_time_from(xmlDocPtr doc, const char *base, const char *path,
                             time_t *value) {
  char *xpath = NULL;
  size_t xpath_size;
  int retval;
...
int pusb_xpath_get_int_from(xmlDocPtr doc, const char *base, const char *path,
                            int *value) {
  char *xpath = NULL;
  size_t xpath_size;
  int retval;
